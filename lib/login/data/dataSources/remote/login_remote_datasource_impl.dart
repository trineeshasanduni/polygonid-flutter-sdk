  ...
  Future<LoginModal> loginDone() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            'loginbody11: ${login.body?.scope![0].query?.credentialSubject?.auth?.$eq}');
                ...
                Scope(
                  ...
                  query: Query(
                    ...
                    credentialSubject: CredentialSubject(
                      auth: Auth(
                        $eq: login.body?.scope![0].query?.credentialSubject
                            ?.auth?.$eq,
                      ),
                    ),
    ...
    while (statusCode != 200) {
      try {
        final response = await client
            .get(Uri.parse('$BASE_URL/status?sessionId=$sessionId'));
        print('login status1: ${response.statusCode}');
