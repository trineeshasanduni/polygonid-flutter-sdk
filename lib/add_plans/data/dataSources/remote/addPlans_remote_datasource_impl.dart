...
abstract interface class AddPlansRemoteDatasource {
  ...
  Future<FreeSpaceModel> freeSpace(
...
class AddPlansRemoteDatasourceImpl implements AddPlansRemoteDatasource {
  ...
  Future<AddPlansModel> verifyUser({
    ...
    try {
      ...
      List<List<String>> bList =
          B.map((e) => e.map((e) => e.toString()).toList()).toList();
  ...
  Future<PriceModel> getPlanPrice(
    ...
    try {
      Map<String, dynamic> data = {
        "Plan": Plan,
        "Month": Month, // Using the TXHash from addUser here
      };
      print('plan data: $data');

      final uri = Uri.parse('$BASE_URL/get-plan-price');
      final response = await http.post(
        uri,
        headers: {"Content-Type": "application/json"},
        body: jsonEncode(data),
      );
      print('plan status code: ${response.statusCode}');

      Map<String, dynamic> jsonResponse = jsonDecode(response.body);

      final responsePlan = PriceModel.fromJson(jsonResponse);
      final planResponse = PriceModel(
        price: responsePlan.price,
      );

      if (response.statusCode == 200) {
        print('plan response: ${planResponse.price}');
        return planResponse;
      } else {
